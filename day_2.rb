...
# they need you to calculate the spreadsheet's checksum. For each row, determine the difference between the largest value and the smallest value; the checksum is the sum of all of these differences.
#
...
# where the result of the division operation is a whole number. They would like you to find those numbers on each line, divide them, and add up each line's result.
#
# For example, given the following spreadsheet:
#
# 5 9 2 8
...
def divisible(array)
  values = array.permutation(2).detect { |values| values[0] % values[1] == 0  }
  values.max / values.min
end

matrix = []

CSV.foreach("day_2.csv", col_sep: "\t", converters: [CSV::Converters[:integer]]) do |row|
  matrix << row
end

matrix.reduce(0) { |sum, arr|  sum + divisible(arr)  }
